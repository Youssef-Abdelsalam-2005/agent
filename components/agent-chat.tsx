...
import { Bot, SendHorizontal, User, MessageSquare, ChevronDown, ChevronRight, FileText, Database } from "lucide-react"
      ...
      toolCalls: [
        {
          ...
          resources: [
            ...
            },
            {
              ...
              snippet: "Marketing: $2.5M, Development: $1.8M, Operations: $1.2M, with contingency reserves of $500K...",
              href: "#",
            },
          ],
        },
                  ...
                  resources: [
                    {
                      ...
                      snippet: "This document outlines the key objectives and deliverables for the upcoming project...",
                      href: "#",
                    },
                    {
                      title: "Design System.png",
                      snippet: "Visual representation of our component library and design tokens...",
                      href: "#",
                    },
                  ],
                },
            ...
            <ScrollArea className="h-[400px]">
              <div className="space-y-2">
                {filteredChats.map((chat) => (
                  ...
                  >
                    <h4 className="font-medium text-zinc-100 truncate">{chat.title}</h4>
                    <p className="text-sm text-zinc-400 truncate mt-1">{chat.lastMessage}</p>
                    <p className="text-xs text-zinc-500 mt-2">{formatTimeAgo(chat.timestamp)}</p>
                  </button>
                ))}
              </div>
            </ScrollArea>
...
function MessageBubble({ message }: { message: Message }) {
  ...
  return (
    <div className={cn("flex gap-3", isUser ? "flex-row-reverse" : "flex-row")}>
      ...
      >
        {isUser ? <User className="size-4 text-orange-400" /> : <Bot className="size-4 text-zinc-300" />}
      </div>
      ...
      <div className={cn("max-w-[80%] space-y-3", isUser ? "items-end" : "items-start")}>
        <div
          className={cn(
            ...
            isUser ? "bg-orange-500/90 text-black" : "bg-zinc-800/0 text-zinc-100",
          )}
        >
          {message.content}
        </div>
      ...
      <CollapsibleContent className="mt-2">
        <div className="bg-zinc-900/60 rounded-lg p-3 text-xs">
          <div className="flex items-center gap-2 mb-2 text-zinc-300">
            ...
            <span className="font-medium">Found {toolCall.resources?.length || 0} resources</span>
          </div>
          {toolCall.resources && (
            <div className="space-y-2">
              {toolCall.resources.map((resource, i) => (
                <div key={i} className="bg-zinc-800/60 rounded p-2">
                  <div className="font-medium text-zinc-200 mb-1">{resource.title}</div>
                  <div className="text-zinc-400 text-[11px] leading-relaxed">{resource.snippet}</div>
                </div>
              ))}
            </div>
          )}
